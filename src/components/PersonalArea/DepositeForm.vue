...
export default {
  ...
  methods: {
    onSubmit (evt) {
      evt.preventDefault()
      this.$http.post("http://localhost:9090/bank/deposit?" +  'var=' + this.depVar + '&' + 'amount=' + this.form.amount + '&' + 'password=' + this.form.password).then(result => {
        if (result.ok) {
